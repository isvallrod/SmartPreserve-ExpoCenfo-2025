...
// === Pines de LEDs (ACTUALIZADOS A TUS PINES REALES) ===
#define LED_GREEN_PIN 4   // LED Verde - Temperatura óptima (TU PIN)
#define LED_YELLOW_PIN 5  // LED Amarillo - Advertencia (TU PIN)
#define LED_RED_PIN 18    // LED Rojo - Crítico (TU PIN)
...
void setup() {
  ...
  pinMode(LED_GREEN_PIN, OUTPUT);   // Pin 4
  pinMode(LED_YELLOW_PIN, OUTPUT);  // Pin 5
  pinMode(LED_RED_PIN, OUTPUT);     // Pin 18
...
void updateLEDs() {
  if (WiFi.status() == WL_CONNECTED) {
      ...
      if (httpCode == 200) {
          String response = http.getString();
          Serial.println("📥 Raw JSON Response from Server: " + response); // <-- ADD THIS LINE
          ...
          if (!error) {
              ...
              // === Actualizar LEDs físicos en TUS PINES ===
              ...
              // Parpadear LED amarillo (Pin 5) para indicar error de parsing
          ...
          // En caso de error HTTP, parpadear LED amarillo (Pin 5)
          digitalWrite(LED_YELLOW_PIN, HIGH);
          delay(200);
          digitalWrite(LED_YELLOW_PIN, LOW);
      }
